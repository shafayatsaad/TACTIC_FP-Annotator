...
export default function IntentLabels({
  ...
  return (
    <div className="shrink-0 rounded-xl p-4 glass-card">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center gap-2 px-2 py-1.5 rounded-md text-left transition-all ${
                        isDisabled
                          ? "cursor-not-allowed glass-btn opacity-40"
                          : isSelected
                            ? "glass-btn shadow-[inset_2px_0_0_0_currentColor]"
      ...
      {onSubmit && (
        <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-3 sticky bottom-0" style={{ backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", background: "rgba(0,0,0,0.4)" }}>
          <button
            onClick={onSubmit}
            className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-lg glass-btn-indigo text-white text-[11px] font-semibold transition-all cursor-pointer hover:scale-[1.02] hover:shadow-[0_4px_20px_rgba(99,102,241,0.3)]"
            title="Submit annotation (Enter)"
          >
            <Check className="w-3.5 h-3.5" />
            Submit
            <kbd className="ml-1 glass-btn px-1.5 py-0.5 rounded text-[9px] font-mono text-indigo-200">
              Enter
            </kbd>
          </button>
        </div>
