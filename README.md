...
| 🤖 | **Auto-segmentation** | Press **O** at any playhead position to instantly create a timed segment. Segments > 15 s are auto-split. |
| 🧠 | **Possession-aware** | Attack intents are disabled for the team without the ball; contested possession is flagged automatically. |
...
- **Auto-split** — segments longer than 15 s are automatically split into 15 s chunks with the same tactical label
- **Manual possession override** — Team A / B / Contested / Auto-follow with live UI feedback
- **Quality gate** — submits blocked unless ≥ 18/22 players are tracked and clip quality ≥ 0.8
- **Forced breaks** every 20 clips and a 50-annotation session cap to maintain annotator accuracy
...
- `pipeline.py` generates contiguous clip windows from `raw_videos/`, writes `data/clip_manifest.json` + `.npz` trajectory files
...
| `POST` | `/api/export/json` | Write JSON export | `{ annotations }` |
...
| `--no-trajectories` | off | Skip `.npz` generation (faster) |
...
interface Annotation {
  ...
  video_source: { video_path; seek_start_sec; label_start_sec; label_end_sec; seek_end_sec };
  segment_metadata: { start_sec; end_sec; duration_sec; coverage_estimate; is_mixed_phase };
  team_a: { label: { intent_class; confidence; certainty }; possession };
  team_b: { /* same shape */ };
  exclusion: "DeadBall" | "ContestedPlay" | null;
  annotation_meta: { annotator_id; session_id; annotation_timestamp; annotation_duration_sec };
...
*A keyboard-first, research-grade football annotation platform.*
